...
class VQModel(pl.LightningModule):
    ...
    def encode(self, x: Tensor) -> Tuple[Tensor, Tensor, Tuple[Any, Any, Tensor]]:
        h = self.encoder(x)
        h = self.quant_conv(h)
    ...
    def get_input(self, batch: Dict[str, Tensor], k: str) -> Tensor:
        x = batch[k]
        if len(x.shape) == 3:
            x = x[..., None]
    ...
    def training_step(self, batch: Dict[str, Tensor], batch_idx: int, optimizer_idx: int) -> Tensor:
    ...
    def _validation_step(self, batch: Dict[str, Tensor], batch_idx: int, suffix: str = "") -> Dict[str, Any]:
        x = self.get_input(batch, self.image_key)
        xrec, qloss, ind = self(x, return_pred_indices=True)
        aeloss, log_dict_ae = self.loss(
            qloss, x, xrec, 0,
    ...
    def configure_optimizers(self) -> Union[List[torch.optim.Optimizer], Tuple[List[torch.optim.Optimizer], List[Dict[str, Any]]]]:
    ...
    def log_images(self, batch: Dict[str, Tensor], only_inputs: bool = False, plot_ema: bool = False, **kwargs: Any) -> Dict[str, Tensor]:
        log = {}
        x = self.get_input(batch, self.image_key)
        x = x.to(self.device)
        if only_inputs:

...
class SafetensorsMapping(typing.Mapping):
    """Mapping class for Safetensors file handling."""
...
# Configuration dictionaries for models
CLIPL_CONFIG = {
...
class Sd3ClipLG(sd_hijack_clip.TextConditionalModel):
    """Class for handling CLIP model interactions."""
...
class Sd3T5(torch.nn.Module):
    ...
    def tokenize_line(self, line, *, target_token_count=None):
        """Tokenizes a single line of text with optional target token count."""
        parsed = prompt_parser.parse_prompt_attention(line) if shared.opts.emphasis != "None" else [[line, 1.0]
        ]
        ...
        if target_token_count is not None:
            if len(tokens) < target_token_count:
                ...
                multipliers.extend([1.0] * (target_token_count - len(tokens)))
            else:
                tokens = tokens[:target_token_count]
                multipliers = multipliers[:target_token_count]

        return tokens, multipliers

    def forward(self, texts, *, token_count):
        """Processes input texts through the T5 model."""
        ...
        tokens_batch = [self.tokenize_line(text, target_token_count=token_count) for text in texts]
...
class SD3Cond(torch.nn.Module):
    """Class for handling conditioning in SD3."""
    ...
    def forward(self, prompts: list[str]):
        """Processes prompts through the models."""
    ...
    def before_load_weights(self, state_dict):
        """Handles loading weights for the models."""
    ...
    def medvram_modules(self):
        """Returns modules for memory optimization."""
    ...
    def get_target_prompt_token_count(self, token_count):
        """Returns the target prompt token count."""

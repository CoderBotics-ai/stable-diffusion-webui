...
class ExtraNetworksPageLora(ui_extra_networks.ExtraNetworksPage):
    ...
    def create_item(self, name: str, index: Optional[int] = None, enable_filter: bool = True) -> Optional[Dict[str, Any]]:
        ...
        # Create item dictionary with type hints
        item: Dict[str, Any] = {
            "name": name,
        ...
        preferred_weight = user_metadata.get("preferred weight", 0.0)
        
        # Using f-strings for better string formatting
        weight_expr = str(preferred_weight) if preferred_weight else "opts.extra_networks_default_multiplier"
        item["prompt"] = f"{quote_js(f'<lora:{alias}:')} + {weight_expr} + {quote_js('>')}"

        if activation_text:
            item["prompt"] += f" + {quote_js(' ' + activation_text)}"

        negative_prompt = user_metadata.get("negative text", "")
        item["negative_prompt"] = quote_js("") if not negative_prompt else quote_js(f'({negative_prompt}:1)')

        # Using pattern matching for SD version handling
        sd_version = user_metadata.get("sd version")
        ...
        if shared.opts.lora_show_all or not enable_filter or not shared.sd_model:
            ...
            model_version = (network.SdVersion.SDXL if shared.sd_model.is_sdxl 
    ...
    def list_items(self) -> Iterator[Dict[str, Any]]:
        # Using list to protect against concurrent modification

...
class ModuleTypeLora(network.ModuleType):
    def create_module(self, net: network.Network, weights: network.NetworkWeights) -> Optional['NetworkModuleLora']:
        """Create a LoRA network module based on weights configuration."""
        match weights.w:
            case w if all(x in w for x in ["lora_up.weight", "lora_down.weight"]):
                return NetworkModuleLora(net, weights)
            case w if all(x in w for x in ["lora_A.weight", "lora_B.weight"]):
                w_copy = w.copy()
                weights.w.clear()
                weights.w.update({
                    "lora_up.weight": w_copy["lora_B.weight"],
                    "lora_down.weight": w_copy["lora_A.weight"]
                })
                return NetworkModuleLora(net, weights)
            case _:
                return None
...
class NetworkModuleLora(network.NetworkModule):
    up_model: nn.Module
    down_model: nn.Module
    mid_model: Optional[nn.Module]
    dim: int
    ...
    def create_module(self, weights: Dict[str, torch.Tensor], key: str, none_ok: bool = False) -> Optional[nn.Module]:
        ...
        supported_conv = (torch.nn.Conv2d,)

        is_linear = isinstance(self.sd_module, supported_linear)
        is_conv = isinstance(self.sd_module, supported_conv)

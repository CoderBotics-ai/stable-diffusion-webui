...
class SdVersion(enum.Enum):
    """Enumeration of supported SD versions"""
    Unknown = 1
    SD1 = 2
    SD2 = 3
    SDXL = 4
...
class NetworkOnDisk:
    def __init__(self, name: str, filename: str | Path) -> None:
        self.name: str = name
        ...
        if self.is_safetensors:
            try:
                self.metadata = cache.cached_data_for_file('safetensors-metadata', "lora/" + self.name, self.filename, read_metadata)
            except Exception as e:
                errors.display(e, f"reading lora {self.filename}")
    ...
    def detect_version(self) -> SdVersion:
        match (self.metadata.get('ss_base_model_version', ""), str(self.metadata.get('ss_v2', ""))):
...
class NetworkModule:
    ...
    def apply_weight_decompose(self, updown: Tensor, orig_weight: Tensor) -> Tensor:
    ...
    def finalize_updown(self, updown: Tensor, orig_weight: Tensor, output_shape: tuple, ex_bias: Optional[Tensor] = None) -> tuple[Tensor, Optional[Tensor]]:
    ...
    def calc_updown(self, target: Tensor) -> tuple[Tensor, Optional[Tensor]]:

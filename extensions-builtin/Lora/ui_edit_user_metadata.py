...
def is_non_comma_tagset(tags: dict[str, Any]) -> bool:
    """Check if the tags represent a non-comma separated tagset."""
    if not tags:
        return False
...
class LoraUserMetadataEditor(ui_extra_networks_user_metadata.UserMetadataEditor):
    ...
    def get_metadata_table(self, name: str) -> list[tuple[str, str]]:
        ...
        if (ss_bucket_info := metadata.get("ss_bucket_info")) and "buckets" in ss_bucket_info:
    ...
    def put_values_into_components(self, name: str) -> list:
        user_metadata = self.get_user_metadata(name)

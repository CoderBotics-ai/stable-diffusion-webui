...
def create_sampler(name: Optional[str], model: Any) -> Any:
...
def get_sampler_and_scheduler(
    ...
    *,
    ...
    if convert_automatic and sampler.options.get('scheduler') == found_scheduler.name:
...
def fix_p_invalid_sampler_and_scheduler(p: Any) -> None:
    ...
    if p.sampler_name != i_sampler_name or i_scheduler != p.scheduler:
        logging.warning(
            f'Sampler Scheduler autocorrection: "{i_sampler_name}" -> "{p.sampler_name}", '

...
def get_learned_conditioning_prompt_schedules(prompts: Sequence[str], base_steps: int, hires_steps: int | None = None, use_old_scheduling: bool = False) -> list[list[tuple[int, str]]]:
    """
    ...
    def at_step(step: int, tree: lark.Tree) -> str:
        class AtStep(lark.Transformer):
            def scheduled(self, args: list[Any]) -> Any:
                before, after, _, when, _ = args
                yield before or () if step <= when else after
            
            def alternate(self, args: list[Any]) -> Any:
                args = ["" if not arg else arg for arg in args]
                yield args[(step - 1) % len(args)]
            
            def start(self, args: list[Any]) -> str:
            ...
            def plain(self, args: list[Any]) -> Any:
                yield args[0].value
...
class SdConditioning(list):
    ...
    """
    def __init__(self, prompts: Sequence[str], is_negative_prompt: bool = False, 
                 width: int | None = None, height: int | None = None, 
                 copy_from: Any | None = None) -> None:
        super().__init__()
        self.extend(prompts)
...
# Rest of the file remains unchanged as it doesn't require version-specific updates
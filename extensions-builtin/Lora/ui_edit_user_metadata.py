...
def build_tags(metadata: Dict[str, Any]) -> List[Tuple[str, int]]:
    """Build and sort tags from metadata."""
    tags: Dict[str, int] = {}
...
class LoraUserMetadataEditor(ui_extra_networks_user_metadata.UserMetadataEditor):
    ...
    def get_metadata_table(self, name: str) -> List[Tuple[str, str]]:
        """Get formatted metadata table for display."""
        ...
        for key, label in keys.items():
            ...
            if value is not None and str(value) != "None":
                table.append((label, html.escape(str(value))))

        ss_training_started_at = metadata.get('ss_training_started_at')
        if ss_training_started_at:
            timestamp = float(ss_training_started_at)
            formatted_date = datetime.datetime.utcfromtimestamp(timestamp).strftime('%Y-%m-%d %H:%M')
            table.append(("Date trained:", formatted_date))
        ...
        if ss_bucket_info and "buckets" in ss_bucket_info:
            ...
            if len(resolutions) > 4:
                resolutions_text += ", ..."
                full_list = html.escape(", ".join(resolutions_list))
                resolutions_text = f"<span title='{full_list}'>{resolutions_text}</span>"
    ...
    def put_values_into_components(self, name: str) -> List[Any]:
        """Update UI components with values from metadata."""
        user_metadata = self.get_user_metadata(name)
    ...
    def generate_random_prompt(self, name: str) -> str:
        """Generate a random prompt based on metadata tags."""
        item = self.page.items.get(name, {})
        metadata = item.get("metadata") or {}
        tags = build_tags(metadata)
    ...
    def generate_random_prompt_from_tags(self, tags: List[Tuple[str, int]]) -> str:
        ...
        for tag, count in tags:
            ...
            if count > v:
                escaped_tag = ''.join(f'\\{x}' if x in "({[]})" else x for x in tag)
